...
def evaluate(
    ...
    """Run inference + classification report on dataset with 'claim', 'evidences', 'lang' fields."""
    ...
    for ex in tqdm(data):
        try:
            ...
            prompt = build_prompt(claim=claim, evidences=evidences, lang_code=lang)
    ...
    with open(
        Path(output_dir) / f"{model_shortname}_predictions_zeroshot26.json", "w", encoding="utf-8"
    ) as f_pred:
        json.dump(results_dump, f_pred, indent=2, ensure_ascii=False)

    with open(
        Path(output_dir) / f"{model_shortname}_metrics_zeroshot26.txt", "w", encoding="utf-8"
    ) as f_met:
        f_met.write(report)
